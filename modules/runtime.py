...
def runtime_to_minutes(runtime_str, round_up = True):
    ...
    logging.debug(f'Converting runtime: {runtime_str}')
    if 'h' in runtime_str:
    ...
    if round_up:
        logging.debug(f'Rounding: {str(runtime_minutes + 15 - (runtime_minutes % 15))=}') # TODO: combine duplicate code into new var
